...
def pvanet_scope(is_training=True,
...
def pvanet(net, include_last_bn_relu = True):
    ...
    end_points['pool2'] = conv2#conv2_3/add:0 (?, 56, 56, 64)
    end_points['pool3'] = conv3#conv3_4/add:0 (?, 28, 28, 128)
    end_points['pool4'] = conv4#conv4_4/incep/add:0 (?, 14, 14, 256)
    end_points['pool5'] = conv5#conv5_4/incep/add:0 (?, 7, 7, 384)

    return conv5, end_points

if __name__ == '__main__':
    input = tf.placeholder(tf.float32, shape=(None, 224, 224, 3), name='input')
    with slim.arg_scope(pvanet_scope()) as sc:
        logits = pvanet(input)

# pool1:0 (?, 56, 56, 32)
# conv2_3/add:0 (?, 56, 56, 64)
# conv3_4/add:0 (?, 28, 28, 128)
# conv4_4/incep/add:0 (?, 14, 14, 256)
# conv5_4/incep/add:0 (?, 7, 7, 384)